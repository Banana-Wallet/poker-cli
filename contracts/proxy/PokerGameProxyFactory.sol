...
contract PokerGameProxyFactory {
    ...
    function proxyCreationCode() public pure returns (bytes memory) {
        return type(PokerGameProxy).creationCode;
    }
    ...
    function deployProxy(
        ...
        bytes32 salt
    ) internal returns (PokerGameProxy proxy) {
        require(isContract(_singleton), 'Singleton contract not deployed');

        bytes memory deploymentData = abi.encodePacked(
            type(PokerGameProxy).creationCode,
    ...
    function createProxyWithNonce(
        ...
        bytes memory initializer,
        uint256 saltNonce
    ) public returns (PokerGameProxy proxy) {
    ...
    function getBytecode(address _owner, uint _foo) public pure returns (bytes memory) {
        bytes memory bytecode = type(PokerGameProxy).creationCode;
